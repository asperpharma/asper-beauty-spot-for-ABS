...
beforeAll(() => {
  type GlobalWithIO = typeof globalThis & { IntersectionObserver: unknown };
  (globalThis as GlobalWithIO).IntersectionObserver = class {
    observe() {}
    unobserve() {}
    disconnect() {}
    constructor() {}
  ...
  return {
    ...
    motion: {
      ...
      div: React.forwardRef(function MockMotionDiv(props: Record<string, unknown>, ref: React.Ref<HTMLDivElement>) {
        ...
        const { variants: _variants, initial: _initial, whileInView: _whileInView, viewport: _viewport, animate: _animate, exit: _exit, ...rest } = props;
...
const mockFrom = () => {
  const chain = {
    select(): typeof chain { return chain; },
    eq(): typeof chain { return chain; },
    ...
    maybeSingle: () => Promise.resolve({ data: TEST_PRODUCT, error: null }),
...
describe("ProductDetail — Key Clinical Actives Animation", () => {
  ...
  it("container uses staggerChildren=0.15, delayChildren=0.1, viewport.once=true", async () => {
    ...
    const container = motionDivCalls.find(
      (c) => (c.variants as AnimVariants)?.visible?.transition?.staggerChildren !== undefined,
    );
    expect(container).toBeDefined();

    const v = container!.variants as AnimVariants;
    ...
    expect((container!.viewport as ViewportConfig).once).toBe(true);
    expect((container!.viewport as ViewportConfig).margin).toBe("-100px");
  });
  ...
  it("each card has y:30, scale:0.98 hidden state and premium ease [0.25,0.1,0.25,1]", async () => {
    ...
    const cards = motionDivCalls.filter((c) => (c.variants as AnimVariants)?.hidden?.y !== undefined);
    expect(cards.length).toBe(3);

    for (const card of cards) {
      const v = card.variants as AnimVariants;
